...
const UserContent = ({ evalList, latestEval, troop, loading, setNames }) => {
  const [showHome, setShowHome] = useState(true);


  const handleClick = (first, last) => {
    setNames(first, last)
    setShowHome(false);
  };
  if (loading) {
  ...
  return (
    <div className="h-fit">
      {showHome && (
        <div>
          {latestEval && (
            <div className='flex mt-20 ml-80 justify-center h-full'>
            <div className='w-1/2'>
              <Card className=" w-fit h-full ml-40 mr-20">
...
export default function Supervisor({ user }) {
  ...
  return (
    ...
    <div>
      <Routes>
        <Route path='/' element={<UserContent evalList={evalList} latestEval={latestEval} troop={troop} loading={loading} setNames={setNames}/>} />
        <Route path='EvalHistory' element={<EvalHistory evalList={evalList} latestEval={latestEval} user={user} />} />
        <Route path='troop/:id/*' element={<TroopPage latestEval={latestEval} supervisor_id={user.id} first={firstName} last={lastName}/>} />
      </Routes>
    </div>
  );
}
